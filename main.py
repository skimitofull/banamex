...
class BanamexPDF(FPDF):
    ...
    def add_row(self, fecha, concepto, retiros, depositos, saldo):
        ...
        # Dibuja franja gris/blanco o blanca si es la primera fila de página
        # 👇 SOLO CAMBIÉ ESTA PARTE
        if self.rows_in_page == 0 and self.page_no_global > 0:
            # Primera fila de página siguiente → blanco
            self.set_fill_color(255, 255, 255)
        elif self.row_global == 0:
            # Primera fila de la primera página → blanco
            self.set_fill_color(255, 255, 255)
        else:
            # Mantiene el patrón intercalado
        ...
        self.set_xy(X_COLS_PT[0], y + (row_height / 2) - 5)
